...
if ($action === 'place') {
    ...
    try {
        $oid = DB::exec(
            "INSERT INTO orders (user_id, name, mobile, district, upazila, union_name, village, road_name, holding_number,
             total_amount, delivery_charge, payment_method, payment_status, sender_last4, status, created_at)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', NOW())",
            [
                ...
                $body['payment_method'] === 'cod' ? 'paid' : 'pending',
                $body['sender_last4']    ?? '',
            ]
        );
        ...
        foreach ($items as $item) {
            DB::run(
                "INSERT INTO order_items (order_id, product_id, product_image_id, product_name, image_path, quantity, price,
                 selected_options, custom_design_text, custom_design_image1, custom_design_image2, created_at)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())",
                [
                    $oid,
    ...
    // Admin only
    require_once __DIR__ . '/../auth.php';
    if (!is_admin()) { echo json_encode(['error' => 'unauth']); exit; }
    $id     = (int)($_GET['id'] ?? 0);
