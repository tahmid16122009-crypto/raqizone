...
if ($action === 'add') {
    ...
    $ex = DB::row(
        "SELECT id, quantity FROM cart_items WHERE user_id = ? AND product_image_id = ?",
        [$u['user_id'], $product_image_id]
    );
    if ($ex) {
        DB::run(
            "UPDATE cart_items SET quantity = ? WHERE id = ?",
            [$ex['quantity'] + $quantity, $ex['id']]
        );
    } else {
        DB::run(
            "INSERT INTO cart_items (user_id, product_id, product_image_id, product_name, image_path, quantity, price, selected_options, created_at)
             VALUES (?, ?, ?, ?, ?, ?, ?, '{}', NOW())",
